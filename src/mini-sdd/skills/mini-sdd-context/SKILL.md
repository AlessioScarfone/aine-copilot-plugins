...
Create or update the project context document at `{SHARED_SUBFOLDER}/context.md`.
...
1. Check whether `{SHARED_SUBFOLDER}/context.md` already exists.
...
- Write the file to `{SHARED_SUBFOLDER}/context.md`
- Set `Last updated` to today's date (`YYYY-MM-DD`)
- Confirm to the user:
  > "✅ Created `{SHARED_SUBFOLDER}/context.md`. You can review and edit it manually anytime."
...
1. Read the current `{SHARED_SUBFOLDER}/context.md`.
2. Ask the user:
...
6. Confirm:
   > "✅ Updated `{SHARED_SUBFOLDER}/context.md`."
