...
Create or update a feature specification file in `{SPECS_SUBFOLDER}/`.
...
1. Read `{SHARED_SUBFOLDER}/context.md` if it exists — use it as background for all decisions.
...
4. Check whether `{SPECS_SUBFOLDER}/<spec-name>.md` already exists.
   - **If it exists** → read the YAML frontmatter to get the current `status`, show a summary and ask:
     > "A spec named `<spec-name>` already exists (status: <current-status>). Do you want to **update this spec** or **create a new one** with a different name?"
...
- Write the file to `{SPECS_SUBFOLDER}/<spec-name>.md`
- In the YAML frontmatter set: `status: todo`, `created: YYYY-MM-DD`, `updated: YYYY-MM-DD`
- Confirm to the user:
  > "✅ Created spec `{SPECS_SUBFOLDER}/<spec-name>.md` (status: todo)."
...
- **Spec file not writable / path missing**: Create the `{SPECS_SUBFOLDER}/` directory if it does not exist before writing.
