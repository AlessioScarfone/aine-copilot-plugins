...
This covers:
- Updating `aine-docs/prd.md` with any new requirements - if update is needed
- Updating `aine-docs/ux.md` and HTML prototype with UX changes - if exists
- Updating `aine-docs/architecture.md` with technical changes - if exists
...
**Input**: The argument after `/aine-propose` is the change name (kebab-case) OR a description of what the user wants to build.

**Agent**: Coordinates the full pipeline, dispatching subagents for global document updates.

**Steps**

0. **PRD Gate** — check if `aine-docs/prd.md` exists.

   If it does NOT exist, stop immediately and tell the user:
   > "A PRD is required before proposing a change. Please create one first using `/aine-prd`, then come back to `/aine-propose`."
...
1. **Understand the change**
   ...
   Read all global project documents (`aine-docs/prd.md`, `aine-docs/ux.md`, `aine-docs/architecture.md`, `aine-docs/prototype-*.html`, `README.md`), existing specs (`aine-docs/specs/*/spec.md`) and user input to fully understand the change and its implications.
...
2. **Phase 1 — Update PRD** (`aine-docs/prd.md`)
...
3. **Phase 2 — Update UX** (`aine-docs/ux.md` + `aine-docs/prototype-*.html`)
...
4. **Phase 3 — Update Architecture** (`aine-docs/architecture.md`)
...
5. **Phase 4 — Create change artifacts**
   ...
   ```bash
   mkdir -p aine-docs/changes/"<name>"
   ```
   ...
   d. **`specs/<capability>/spec.md`** — only if new capabilities are introduced
      ...
      - Save to `aine-docs/changes/<name>/specs/<capability>/spec.md`

6. **Completion**

   Show pipeline summary:
   - Which global docs were updated and what changed in each
   - List of artifacts created under `aine-docs/changes/<name>/`
   - Status: "All artifacts created! Ready for implementation with `/aine-implement`."
